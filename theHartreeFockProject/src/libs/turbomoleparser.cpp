...
bool TurboMoleParser::load(string fileName)
{
    ...
    while (getline(dataFile, line))
    {
        ...
        if(regex_search(line, what, exponentWeightRegex)) {
            ...
            weights.set_size(Nprimitives);
            exponents.set_size(Nprimitives);
            weights(Nprimitives) = weight;
            exponents(Nprimitives) = exponent;
